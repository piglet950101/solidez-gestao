-- Bugfix (continuação): com a constraint deferrable corrigida, veio à tona um
-- segundo bug latente da fn_gerar_alertas_diarios original: os blocos
-- doc_veiculo / fim_experiencia / imposto_pendente usavam CASE com texto puro
-- ('vermelho'/'amarelo'/'verde') sem cast pra alerta_severidade. Uma expressão
-- CASE resolve pra text, e não há cast implícito text->enum em INSERT.
-- (Os literais soltos de `tipo` funcionam porque são 'unknown', não 'text'.)
--
-- Redefine a função com ::alerta_severidade explícito em todos os CASE.
...
begin
  ...
  select 'doc_veiculo'::alerta_tipo,
         case when v.doc_vencimento < current_date then 'vermelho'::alerta_severidade
              when v.doc_vencimento <= current_date + 30 then 'amarelo'::alerta_severidade
              else 'verde'::alerta_severidade end,
  ...
  insert into alertas (tipo, severidade, empresa_id, entidade_tabela, entidade_id, mensagem)
  select 'fim_experiencia'::alerta_tipo,
         case when (current_date - f.data_admissao) between 38 and 45 then 'amarelo'::alerta_severidade
              when (current_date - f.data_admissao) between 83 and 90 then 'vermelho'::alerta_severidade
              else 'verde'::alerta_severidade end,
  ...
  insert into alertas (tipo, severidade, empresa_id, entidade_tabela, entidade_id, mensagem)
  select 'imposto_pendente'::alerta_tipo, 'amarelo'::alerta_severidade, i.empresa_id, 'impostos', i.id,
  ...
  -- Documentos / cursos de funcionário vencendo (NR, ASO, etc)
  insert into alertas (tipo, severidade, empresa_id, entidade_tabela, entidade_id, mensagem, contexto)
  select 'doc_funcionario_vencendo'::alerta_tipo,
