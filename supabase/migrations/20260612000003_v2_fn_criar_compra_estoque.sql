-- V2: fn_criar_compra aceita p_tipo_despesa e permite alocações vazias quando estoque

-- Drop overload antigo (assinatura sem p_tipo_despesa) — substituído pela versão V2 abaixo.
drop function if exists fn_criar_compra(uuid,uuid,uuid,text,numeric,date,rateio_modo,quem_pagou_tipo,uuid,uuid,text,text,jsonb,jsonb,uuid,uuid,fase_funcionario);
...
begin
  -- Estoque pode não ter alocações (custo flui via saída do estoque)
  if p_tipo_despesa <> 'estoque' and jsonb_array_length(p_alocacoes) = 0 then
...
comment on function fn_criar_compra(uuid,uuid,uuid,text,numeric,date,rateio_modo,quem_pagou_tipo,uuid,uuid,text,text,jsonb,jsonb,uuid,uuid,fase_funcionario,text) is
  'V2: aceita p_tipo_despesa (individual_obra|administrativa|estoque). Quando estoque, p_alocacoes pode ser vazio — custo flui via saída do estoque.';
